...
4. [x] 实现 `memory-core`：`MemoryError`、数据模型与校验、trait、工具 + 单测
5. [x] 实现 `memory-embed`：`LocalEmbedder`、`cosine` + 单测
6. [x] 实现 `memory-storage`：`SqliteStore` + 单测
7. [x] 实现 `memory`：`MemoryManager`、`lifecycle` + 单测
8. [x] 实现 `aria-memory`：add/get/search/list/forget + 单测
...
12. [x] CLI `memory bench --json`（进程内 add/search 微基准，供 Python 解析）
...
- `cargo run -p aria-memory -- bench --size 100 --json` 输出合法 JSON。
