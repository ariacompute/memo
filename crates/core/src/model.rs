...
impl MemoryType {
    ...
    pub fn as_str(&self) -> String {
        match self {
            ...
            MemoryType::ShortTerm => "short_term".to_string(),
            MemoryType::LongTerm { kind } => match kind {
...
impl std::str::FromStr for MemoryType {
    ...
    fn from_str(s: &str) -> Result<Self> {
        match s {
            ...
            "long_term:episodic" => Ok(MemoryType::LongTerm {
                kind: LongTermKind::Episodic,
            }),
            "long_term:semantic" => Ok(MemoryType::LongTerm {
                kind: LongTermKind::Semantic,
            }),
            "long_term:entity" => Ok(MemoryType::LongTerm {
                kind: LongTermKind::Entity,
            }),
            "long_term:graph" => Ok(MemoryType::LongTerm {
                kind: LongTermKind::Graph,
            }),
            _ => Err(MemoryError::InvalidParam(format!("unknown memory_type: {s}"))),
        }
    }
}
...
impl SearchQuery {
    ...
    pub fn validate(&self) -> Result<()> {
        if self.text.trim().is_empty() {
            return Err(MemoryError::InvalidParam("empty query text".into()));
        }
        if self.top_k == 0 {
            return Err(MemoryError::InvalidParam("top_k must be > 0".into()));
        }
        if !(0.0..=1.0).contains(&self.semantic_weight) {
            return Err(MemoryError::InvalidParam("semantic_weight out of [0,1]".into()));
        }
        if !(0.0..=1.0).contains(&self.keyword_weight) {
            return Err(MemoryError::InvalidParam("keyword_weight out of [0,1]".into()));
        }
        if self.semantic_weight == 0.0 && self.keyword_weight == 0.0 {
            return Err(MemoryError::InvalidParam(
...
mod tests {
    ...
    fn memory_type_roundtrip() {
        for t in [
            ...
            let s = t.as_str();
            assert_eq!(<MemoryType as std::str::FromStr>::from_str(&s).unwrap(), t);
        }
    }

    #[test]
    fn memory_type_invalid() {
        assert!(<MemoryType as std::str::FromStr>::from_str("bogus").is_err());
    }

    #[test]
    fn memory_validate_normal_and_abnormal() {
        assert!(sample().validate().is_ok());

        let mut m = sample();
        m.content = "   ".into();
        assert!(matches!(m.validate(), Err(MemoryError::EmptyContent)));

        let mut m = sample();
        m.importance = 1.5;
        assert!(matches!(m.validate(), Err(MemoryError::InvalidParam(_))));

        let mut m = sample();
        m.embedding = Some(vec![]);
        assert!(matches!(m.validate(), Err(MemoryError::EmptyEmbedding)));
    }
    ...
    fn query_defaults_and_validate() {
        ...
        assert!(matches!(q.validate(), Err(MemoryError::InvalidParam(_))));
